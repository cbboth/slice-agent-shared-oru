...
entity insertion_sort is
  ...
  port (
    ...
    src_tdata   :  in std_logic_vector(DATA_WIDTH - 1 downto 0);
    src_tlast   :  in std_logic;

    dest_tvalid : out std_logic;
    dest_tready :  in std_logic;
    dest_tdata  : out std_logic_vector(DATA_WIDTH - 1 downto 0);
...
architecture rtl of insertion_sort is
  ...
  signal dest_tvalid_cpy : std_logic := '0';
  signal src_tready_cpy  : std_logic := '0';
  signal dest_tlast_cpy  : std_logic := '0';

  type arr_type is array(2**ADDR_WIDTH - 1 downto 0)
    of std_logic_vector(DATA_WIDTH - 1 downto 0);
  signal data_buf : arr_type := (others=>(others=>'0'));

  type sm_type is (SM_SORT, SM_EJECT);
  signal sm : sm_type := SM_SORT;
...
begin
  ...
  src_tready_cpy <= rst when sm = SM_SORT else '0'; 
  dest_tvalid_cpy <= rst when sm = SM_EJECT else '0';
      ...
      if src_tvalid = '1' and src_tready_cpy = '1' then
        case sm is
          when SM_SORT =>
            ...
            elsif(data_buf(0)(46 downto 36) >= src_tdata(46 downto 36)) then
              data_buf(0) <= src_tdata;
            else
            ...
            for i in 1 to data_buf'length - 1 loop
              if (i < cnt) then
                if (data_buf(i - 1)(46 downto 36) >= src_tdata(46 downto 36)) then
                  data_buf(i) <= data_buf(i - 1);
                elsif (data_buf(i)(46 downto 36) >= src_tdata(46 downto 36)) then
